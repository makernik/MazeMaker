...
- **C4** — Center room, start/finish relocation, finish arrow (done); variable wedges + cleanup (pending)
  ...
  - **Variable wedges (done):** `PolarGrid(rings, baseWedges, wedgeMultiplier)` with `wedgesAtRing(r)`; outer rings have baseWedges × multiplier^(r-1) (capped at 64). One inner cell can have two outward neighbors; `walls.OUTWARD` is an array of 1 or 2; `getNeighbor` returns arrays; `removeWallBetween` and solver adapter handle outward index. [draw-polar.js](src/pdf/drawers/draw-polar.js) uses `grid.wedgesAtRing(r)` per ring. Presets have `polarWedgeMultiplier` (1 for 3, 4-5; 2 for 6-8 and above). Tests: polarGrid (variable-wedge ring counts, two outward neighbors), polarGenerator (6-8 perfect maze with variable wedges).
